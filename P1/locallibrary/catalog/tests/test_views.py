...
class AuthorListViewTest(TestCase):
    ...
    def test_lists_all_authors(self):
        # Se ha cambiado este test para que que se adapte a la paginacion de 2. Dando como resultado, q en la pagina 7(la ultima), habra solo 1 autor 
...
class AuthorCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user_no_perms = User.objects.create_user(username='user1', password='password123')

        # Crear usuario con permiso para agregar autores
        self.user_with_perms = User.objects.create_user(username='user2', password='password123')
    ...
    #Verifica que un usuario con permisos pueda acceder y que se use la plantilla correcta.
    def test_logged_in_with_permission(self):
        """Verifica que un usuario con permisos puede acceder a la vista."""
        self.client.login(username='user2', password='password123')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)  # OK
        self.assertTemplateUsed(response, 'catalog/author_form.html')  # Verificar plantilla

    #Vemos que el campo date_of_death tenga la fecha inicial correcta (11/11/2023).
    def test_initial_date_of_death(self):
        """Verifica que el campo `date_of_death` tenga el valor inicial correcto."""
        self.client.login(username='user2', password='password123')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)
    ...
    #Verificamos que se redirige a la página de detalle del autor tras un POST exitoso.
    def test_redirects_to_detail_view_on_success(self):
        """Verifica que tras un POST exitoso, se redirige a la página de detalle del autor."""
        self.client.login(username='user2', password='password123')
        ...
        self.assertTrue(Author.objects.filter(first_name="Julio", last_name="Verne").exists())
