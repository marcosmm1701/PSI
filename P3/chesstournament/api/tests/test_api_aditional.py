...
from chess_models.models import Game, Player, Tournament
from chess_models.models import RankingSystemClass
from chess_models.models import Round, TournamentBoardType, TournamentType
from chess_models.models import Scores, TournamentPlayer, RankingSystem
...
class CreateRoundAPIViewTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        # URL de la API para crear ronda
    ...
    def test_tournament_does_not_exist(self):
        ...
        self.assertEqual(response.data['result'], False)
        self.assertEqual(response.data['message'],
                         "Tournament with id 99999 does not exist")
    ...
    def test_tournament_has_no_players(self):
        ...
        # Verificamos que la respuesta es un error
        # debido a la falta de jugadores
        self.assertEqual(response.status_code,
                         status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['result'], False)
        self.assertEqual(response.data['message'],
                         f"Tournament with id {tournament.id} has no players")

    """
    @tag("continua")
    @patch("chess_models.models.create_rounds",
    side_effect=Exception("Error creating rounds"))
    def test_error_creating_rounds(self, mock_create_rounds):
        ...
        # Intentamos crear rondas cuando la función `create_rounds`
        # lanza una excepción
        response = self.client.post(self.create_round_url,
        {'tournament_id': tournament.id})
        ...
        self.assertEqual(response.data['result'], False)
        self.assertEqual(response.data['message'],
        "An error occurred while creating rounds: Error creating rounds")
    ...
    def test_tournament_has_no_rounds_after_creation(self):
        ...
        # Verificamos que el código de estado sea 400 si
        # no se crean rondas
        self.assertEqual(response.status_code,
        status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['result'], False)
        self.assertEqual(response.data['message'],
        f"Tournament with id {tournament.id} has no rounds")
    ...
    def test_create_rounds_failure(self):
        ...
        # Mockear la función create_rounds para que
        # devuelva -1 (simulando un error)
        ...
        startswith('An error occurred while creating rounds'))
    """
    ...
    def test_create_round_success(self):
        ...
        self.assertEqual(len(tournament.getGames()), (NoItems - 1)
                         * NoItems // 2)
...
class SearchTournamentsAPIViewTest(TransactionTestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.url = '/api/v1/searchTournaments/'
...
class TournamentCreateAPIViewTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.url = '/api/v1/tournament_create/'
    ...
    def test_create_tournament_success(self):
        ...
        self.assertEqual(response.data['tournament_type'],
                         data['tournament_type'])
        self.assertEqual(response.data['board_type'],
                         data['board_type'])
    ...
    def test_create_tournament_with_csv_players(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data['name'], data['name'])
        self.assertEqual(TournamentPlayer.objects.count(), 2)
    ...
    def test_create_tournament_with_lichess_players(self):
        ...
        players = Player.objects.filter(lichess_username__in=["player1",
                                                              "player2"])
        self.assertEqual(players.count(), 2)

        # Verifica que están vinculados al torneo
        t = tournament
        tournament_players = TournamentPlayer.objects.filter(tournament=t)
        self.assertEqual(tournament_players.count(), 2)

        usernames = [tp.player.lichess_username for tp
...
class GetRankingAPIViewTest(TransactionTestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.url_base = '/api/v1/get_ranking/'
...
class continuaAPITest(TransactionTestCase):
    ...
    def test_round_with_games_and_players(self):
        ...
        Game.objects.create(round=rnd, white=player_white,
                            black=player_black, result="W")
    ...
    def test_round_with_lichess_usernames(self):
        ...
        white = Player.objects.create(name="Jugador Blanco",
                                      lichess_username="superGM")
        black = Player.objects.create(name="Jugador Negro",
                                      lichess_username="anotherGM")
...
class UpdateLichessGameAPIViewTest(TransactionTestCase):
    ...
    def test_game_does_not_exist(self):
        """Debe devolver 404 si el game_id no corresponde a un
        juego existente"""
    ...
    def test_game_is_finished_blocked(self):
        """Debe devolver 400 si el juego está terminado y no
        se puede editar"""
...
class UpdateOTBGameAPIViewTest(TransactionTestCase):
    ...
    def test_game_does_not_exist_otb(self):
        """Debe devolver 404 si el game_id no corresponde
    ...
    def test_game_is_finished_otb(self):
        ...
        tournament = Tournament.objects.create(name="Torneo 1",
                                               administrativeUser=None)
        round_obj = Round.objects.create(name="Round 1",
                                         tournament=tournament)
        ...
        data = {"game_id": game.id, "otb_result": "b",
                "email": black.email}
        response = self.client.post(self.url, data)
    ...
    def test_invalid_email_otb(self):
        ...
        data = {"game_id": game.id, "otb_result": "w",
...
class AdminUpdateGameAPIViewTest(TransactionTestCase):
    ...
    def test_invalid_result_value_admin(self):
        ...
        white = Player.objects.create(name="Player 1")
        black = Player.objects.create(name="Player 2")
        tournament = Tournament.objects.create(name="Torneo Admin",
                                               administrativeUser=self.user)
        round_obj = Round.objects.create(name="Ronda 1",
                                         tournament=tournament)
        game = Game.objects.create(white=white, black=black,
                                   round=round_obj)
    ...
    def test_draw_result_normalization_admin(self):
        ...
        round_obj = Round.objects.create(name="Ronda 1",
                                         tournament=tournament)
        game = Game.objects.create(white=white, black=black,
                                   round=round_obj, finished=False)
