...
class TournamentModelTest(TransactionTestCase):
    ...
    def test_002_tournament_player_str(self):
        ...
        referee = Referee.objects.create(name="Referee")
        tournament = Tournament.objects.create(name="Tournament A", referee=referee)
        tp = TournamentPlayer.objects.create(tournament=tournament, player=player)
        self.assertEqual(str(tp), f"{tournament.name} - {player.name}")


    @tag("continua")
    def test_003_get_players_unsorted(self):
        referee = Referee.objects.create(name="Ref")
        tournament = Tournament.objects.create(
            name="T1",
            referee=referee,
            tournament_speed=TournamentSpeed.CLASSICAL,
        ...
        p1 = Player.objects.create(name="A", fide_rating_classical=1500)
        p2 = Player.objects.create(name="B", fide_rating_classical=1600)

        tournament.players.add(p1, p2)
        players = tournament.getPlayers(sorted=False)
        self.assertEqual(set(players), set([p1, p2]))
    ...
    def test_004_get_players_sorted_fide_classical(self):
        ...
        tournament.players.add(p1, p2)
        sorted_players = tournament.getPlayers(sorted=True)
        self.assertEqual(sorted_players, [p2, p1])
    ...
    def test_005_add_duplicate_ranking_to_list(self, mock_stdout):
        referee = Referee.objects.create(name="Ref")
        tournament = Tournament.objects.create(name="T", referee=referee)
        
        tournament.addToRankingList(RankingSystem.PLAIN_SCORE.value)
        # Segundo intento forzado con try/except
        tournament.addToRankingList(RankingSystem.PLAIN_SCORE.value)
    ...
    def test_006_remove_nonexistent_ranking_from_list(self, mock_stdout):
        ...
        tournament.removeFromRankingList("ZZ")  # valor no existente
