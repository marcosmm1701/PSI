...
def reset_sequences(tournament=True,
    ...
    def reset_sequence(app_name, model_name):
        ...
        with connection.cursor() as cursor:
            # Get the name of the sequence associated with the primary key
            # column
            cursor.execute(
                f"SELECT pg_get_serial_sequence(\'{table_name}\', \'{primary_key_column}\');")
            sequence_name = cursor.fetchone()[0]

            if sequence_name:
                if verbose:
                    print(
                        f"Resetting sequence {sequence_name} for {table_name}.{primary_key_column}")
                cursor.execute(
                    f"SELECT setval(\'{sequence_name}\', 1, false) FROM {table_name};")
            else:
                print(
                    f"No sequence found for {table_name}.{primary_key_column}")
    if tournament:
