...
from chess_models.models import Player, Tournament, Round, Game, Referee
...
from chess_models.tests.constants import (
    lichess_usernames_6)
from chess_models.models import (TournamentSpeed, TournamentBoardType,
...
class GameModelTest(TransactionTestCase):
    ...
    def test_005_game_get_result_from_lichess_missing_data(self):
        """
        La función debe retornar None si
        faltan datos como game_id, white o black.
        """
    ...
    def test_006_game_get_result_draw(self, mock_get):
        """Test que verifica que la partida se
        marque como empate (DRAW) si no hay ganador"""
    ...
    def test_007_game_get_result_raises_on_connection_error(self, mock_get):
        """Debe lanzar LichessAPIError si
        falla la conexión con la API de Lichess"""
        game = Game.objects.create(
            round=self.round,
            white=self.players[0],
            black=self.players[1])
        ...
        self.assertIn(
            "Error al conectar con Lichess en game", str(
                context.exception))
    ...
    def test_010_create_rounds_zero_players(self):
        ...
        self.assertEqual(
            Round.objects.filter(
                tournament=tournament).count(), 0)
        self.assertEqual(
            Game.objects.filter(
                round__tournament=tournament).count(), 0)
    ...
    @tag("continua")
    @patch("builtins.print")
    def test_020_create_rounds_zero_players_verify_print_output(
            self, mock_print):
    ...
    def test_021_create_rounds_zero_players_multiple_prints(self, mock_print):
        """Test para verificar múltiples prints
        cuando hay varios checks de 0 jugadores"""
        tournament = Tournament.objects.create(
            name='MultiCheckTournament',
            tournament_type=TournamentType.DOUBLEROUNDROBIN
        )

        # Llamar a create_rounds dos veces para verificar que el print ocurre
        # cada vez
    ...
    def test_022_create_rounds_zero_players_with_swiss_byes(self, mock_print):
        """Test que verifica el print cuando
        hay 0 jugadores pero con swissByes"""
        tournament = Tournament.objects.create(
...
class PlayerModelTest(TransactionTestCase):
    ...
    @tag("continua")
    @patch("requests.get",
           side_effect=requests.exceptions.RequestException("Error simulado"))
    def test_008_get_lichess_user_ratings_connection_error(self, mock_get):
        """Debe lanzar LichessAPIError si falla
        la conexión a Lichess en get_lichess_user_ratings"""
        player = Player(lichess_username="testuser")
        with self.assertRaises(LichessAPIError) as cm:
            player.get_lichess_user_ratings()
        self.assertIn(
            "Nombre de usuario Lichess no proporcionado", str(
                cm.exception))
    ...
    @patch("requests.get",
           side_effect=requests.exceptions.RequestException("Error simulado"))
    def test_009_check_lichess_user_exists_connection_error(
            self, mock_get, mock_print):
        """Debe retornar False y mostrar un mensaje si
        falla la conexión en check_lichess_user_exists"""
        player = Player(lichess_username="testuser")
...
class TournamentModelTest(TransactionTestCase):
    ...
    def test_001_ranking_system_str(self):
        rs = RankingSystemClass.objects.create(
    ...
    def test_002_tournament_player_str(self):
        tournament = Tournament.objects.create(
            name="Tournament A", referee=self.referee)
        tp = TournamentPlayer.objects.create(
            tournament=tournament, player=self.player1)
    ...
    def test_005_add_duplicate_ranking_to_list(self, mock_stdout):
        ...
        self.assertIn(
            "Error: El objeto ya existe en la lista de ranking.",
            output)
        self.assertEqual(
            tournament.rankingList.count(),
            1)  # Solo debería estar una vez
    ...
    def test_006_remove_nonexistent_ranking_from_list(self, mock_stdout):
        ...
        output = mock_stdout.getvalue()
        self.assertIn(
            "No se encontró el ranking con valor 'ZZ' en RankingSystemClass.",
            output)
    ...
    def test_clean_ranking_list(self):
        ...
        rs1 = RankingSystemClass.objects.create(
            value=RankingSystem.PLAIN_SCORE.value)
    ...
    def test_get_scores_white_win_black_empty(self):
        ...
        TournamentPlayer.objects.create(
            player=self.player1, tournament=tournament)
        TournamentPlayer.objects.create(
            player=self.player2, tournament=tournament)

        scores = getScores(tournament)
        self.assertEqual(scores[self.player1][PLAIN_SCORE], 0.0)
        self.assertEqual(scores[self.player2]
                         [PLAIN_SCORE], 0.0)  # player2 no jugó

    @tag("continua")
    def test_015_get_players_count(self):
        tournament = Tournament.objects.create(
            name="PlayerCountTest", referee=self.referee)
        tournament.players.add(self.player1, self.player2)
    ...
    def test_016_get_round_count(self):
        tournament = Tournament.objects.create(
            name="RoundCountTest", referee=self.referee)
    ...
    def test_017_number_of_rounds_with_games(self):
        tournament = Tournament.objects.create(
            name="RoundsWithGames", referee=self.referee)
    ...
    def test_018_latest_round_with_games(self):
        tournament = Tournament.objects.create(
            name="LatestRoundTest", referee=self.referee)
    ...
    def test_021_get_black_wins_stats(self):
        ...
        TournamentPlayer.objects.create(
            player=self.player2, tournament=tournament)
        TournamentPlayer.objects.create(
            player=self.player3, tournament=tournament)
        ...
        self.assertEqual(results[self.player2]
                         [BLACKTIMES], 1)  # Jugó con negras
        self.assertEqual(results[self.player1][WINS], 1)  # Ganó con blancas
        # No jugó con negras
        self.assertEqual(results[self.player1][BLACKTIMES], 0)
    ...
    def test_022_get_ranking_with_criteria(self):
        ...
        # Verificar orden esperado: player2 (1 victoria), player1 (0.5 puntos),
        # player3 (0.5 puntos)
    ...
    def test_025_tournament_creation_with_all_fields(self):
        ...
        self.assertEqual(
            tournament.tournament_speed,
            TournamentSpeed.BLITZ.value)
    ...
    @tag("continua")
    @patch("sys.stdout", new_callable=StringIO)
    def test_add_to_ranking_list_raises_exception_and_prints(
            self, mock_stdout):
        tournament = Tournament.objects.create(
            name="ErrorTest", referee=self.referee)

        # Creamos un ranking_obj válido
        ranking_obj = RankingSystemClass.objects.create(
            value=RankingSystem.PLAIN_SCORE.value)

        ranking_obj = ranking_obj
        # Mockeamos el método 'add' para lanzar una excepción
        with patch.object(tournament.rankingList, 'add',
    ...
    def test_add_to_ranking_list_forces_exception_in_add(self, mock_stdout):
        ...
        # Ahora parcheamos el método `add` del campo `rankingList` para que
        # dispare una excepción
        with patch.object(tournament.rankingList, 'add',
                          side_effect=Exception("Mocked exception on add")):
