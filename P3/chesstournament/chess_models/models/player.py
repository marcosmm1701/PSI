...
class Player(models.Model):
    ...
    # Rating del jugador en la modalidad Bullet en Lichess
    lichess_rating_bullet = models.IntegerField(default=0, null=True)
    # Rating del jugador en la modalidad Blitz en Lichess.
    lichess_rating_blitz = models.IntegerField(default=0, null=True)
    # Rating del jugador en la modalidad Rapid en Lichess.
    ...
    class Meta:
        # La pareja (name, email) debe ser única
        unique_together = ('name', 'email')

    def save(self, *args, **kwargs):
        """Sobrescribe save() para actualizar
        jugadores en lugar de duplicarlos."""
        if not self.id and not self.creation_date:
            self.creation_date = timezone.now()

        existing_player = None

        # Hemos separado las condiciones de jugador existente para evitar
        # errores en los tests de tournament y game
        if self.lichess_username and self.lichess_username != "":
            existing_player = Player.objects.filter(
                models.Q(
                    lichess_username=self.lichess_username)).exclude(
                id=self.id).first()

        if self.fide_id and self.fide_id != "" and not existing_player:
            existing_player = Player.objects.filter(
                models.Q(
                    fide_id=self.fide_id)).exclude(
                id=self.id).first()

        if (self.email and self.email != "") and (
                self.name and self.name != "") and not existing_player:
            existing_player = Player.objects.filter(
                models.Q(
                    email=self.email,
        ...
        if existing_player:
            # Asignamos el ID del jugador existente
            ...
            for field in self._meta.fields:
                ...
                if field_name not in ["id", "creation_date", "update_date"]:
                    setattr(
                        existing_player, field_name, getattr(
                            self, field_name))
        ...
        # Si el jugador tiene un lichess_username,
        # actualizar sus ratings desde
        # Lichess
    ...
    def get_lichess_user_ratings(self):
        ...
        en Lichess y actualiza los valores en la base de datos."""
        if self.check_lichess_user_exists() is False:
        ...
        if response.status_code == 200:
            ...
            self.lichess_rating_bullet = perfs.get(
                "bullet", {}).get("rating", 0)
